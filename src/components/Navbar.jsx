import { useState, useEffect, useRef } from "react";
import OverlayMenu from "./OverlayMenu";
import { FiMenu } from "react-icons/fi";
// import Logo from "../assets/Logo.png"; // Update path if logo is moved

export default function Navbar() {
  // State for controlling the overlay menu
  const [menuOpen, setMenuOpen] = useState(false);

  // State for navbar visibility (show/hide on scroll)
  const [visible, setVisible] = useState(true);

  // State to force navbar always visible on homepage ("#home" section)
  const [forceVisible, setForceVisible] = useState(false);

  // Refs to track last scroll position and timer for auto-hide
  const lastScrollY = useRef(0);
  const timerId = useRef(null);

  // ✅ Detect whether we are on the homepage section
  useEffect(() => {
    const homeSection = document.querySelector("#home");

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          // If "home" section is in view → navbar stays visible
          setForceVisible(true);
          setVisible(true);
        } else {
          setForceVisible(false);
        }
      },
      { threshold: 0.1 } // Adjust % of visibility before triggering
    );

    if (homeSection) observer.observe(homeSection);

    return () => {
      if (homeSection) observer.unobserve(homeSection);
    };
  }, []);

  // ✅ Handle scroll events (show/hide navbar intelligently)
  useEffect(() => {
    const handleScroll = () => {
      if (forceVisible) {
        // Always visible on homepage
        setVisible(true);
        return;
      }

      const currentScrollY = window.scrollY;

      if (currentScrollY > lastScrollY.current) {
        // Scrolling down → hide navbar
        setVisible(false);
      } else {
        // Scrolling up → show navbar
        setVisible(true);

        // Auto-hide after 3 seconds of no scroll
        if (timerId.current) clearTimeout(timerId.current);
        timerId.current = setTimeout(() => {
          setVisible(false);
        }, 3000);
      }

      lastScrollY.current = currentScrollY;
    };

    window.addEventListener("scroll", handleScroll, { passive: true });

    return () => {
      window.removeEventListener("scroll", handleScroll);
      if (timerId.current) clearTimeout(timerId.current);
    };
  }, [forceVisible]);

  return (
    <>
      {/* Top Navbar */}
      <nav
        className={`fixed top-0 left-0 w-full flex items-center justify-between px-6 py-4 z-50 transition-transform duration-300 ${
          visible ? "translate-y-0" : "-translate-y-full"
        }`}
      >
        {/* 🔹 Logo + Name */}
        <div className="flex items-center space-x-2">
          {/* <img src={Logo} alt="Logo" className="w-8 h-8" /> */}
          <div className="text-2xl font-bold text-white hidden sm:block">
            Utkarsh
          </div>
        </div>

        {/* 🔹 Center Menu Button (Hamburger) */}
        <div className="block lg:absolute lg:left-1/2 lg:transform lg:-translate-x-1/2">
          <button
            onClick={() => setMenuOpen(true)}
            className="text-white text-3xl focus:outline-none"
            aria-label="Open menu"
          >
            <FiMenu />
          </button>
        </div>

        {/* 🔹 Right Side Contact Button (Desktop Only) */}
        <div className="hidden lg:block">
          <a
            href="#contact"
            className="bg-gradient-to-r from-pink-500 to-blue-500 text-white px-5 py-2 rounded-full font-medium shadow-lg hover:opacity-90 transition-opacity duration-300"
          >
            Reach Out
          </a>
        </div>
      </nav>

      {/* Overlay menu (slides in when hamburger clicked) */}
      <OverlayMenu isOpen={menuOpen} onClose={() => setMenuOpen(false)} />
    </>
  );
}
